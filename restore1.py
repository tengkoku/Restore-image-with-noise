...
def solution(img):
    ...
    dst = cv.medianBlur(img,5)  #solve saltnpepper  
    dst1 = cv.GaussianBlur(dst,(5,5),1,1,cv.BORDER_DEFAULT) #blur
    img2 = cv.subtract(dst,dst1) #subtract
    img3 = cv.add(dst,img2) #add
